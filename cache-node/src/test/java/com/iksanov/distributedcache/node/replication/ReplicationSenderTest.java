...
class ReplicationSenderTest {
    ...
    void shouldRemoveChannelOnWriteFailure() {
        ...
        assertFalse(sender.channelsMap().containsKey(replicaA.nodeId()),
                "Channel should be removed after write failure");
    }
    ...
    void shouldHandleExceptionsOnShutdown() {
        ...
        assertDoesNotThrow(() -> sender.shutdown(),
                "Shutdown must not throw even if channel.close() fails");

        assertTrue(sender.channelsMap().isEmpty(),
                "Channels map must be cleared even after exceptions");
    }
